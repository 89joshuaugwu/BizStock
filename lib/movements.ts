 ...
 * movements (purchases + sales) across all products, for the dashboard
 * home activity feed. */
export function onRecentMovementsSnapshot(
  count: number,
  callback: (movements: StockMovement[]) => void
): Unsubscribe {
  const q = query(collection(db, "stockMovements"), orderBy("createdAt", "desc"), limit(count));
  return onSnapshot(q, (snap) => {
    callback(snap.docs.map((d) => ({ id: d.id, ...(d.data() as Omit<StockMovement, "id">) })));
  });
}

/** Owner-only. Full stock movement history for a single product — used on
